<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Tambah Lokasi Baru</h2>
    </x-slot>
    <div class="py-8">
            ...
            <div class="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                <form action="{{ route('locations.store') }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="3"
                                class="w-full rounded-xl border-gray-200 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="text-xs text-rose-500 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="mt-8 flex justify-end gap-3 border-t border-gray-100 pt-6">
                        ...
                        <button type="submit"
                            class="px-5 py-2.5 bg-gradient-to-r from-teal-600 to-teal-700 text-white text-xs font-bold rounded-xl shadow-md transition-all">Simpan
                            Lokasi</button>
                    </div>
                </form>
            </div>
